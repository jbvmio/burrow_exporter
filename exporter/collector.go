...
var (
	kafkaConsumerPartitionLagDesc           = prometheus.NewDesc("kafka_burrow_partition_lag", "The lag of the latest offset commit on a partition as reported by burrow.", []string{"cluster", "group", "topic", "partition", "owner", "clientid"}, nil)
	kafkaConsumerPartitionCurrentOffsetDesc = prometheus.NewDesc("kafka_burrow_partition_current_offset", "The latest offset commit on a partition as reported by burrow.", []string{"cluster", "group", "topic", "partition", "owner", "clientid"}, nil)
	kafkaConsumerPartitionCurrentStatusDesc = prometheus.NewDesc("kafka_burrow_partition_status", "The status of a partition as reported by burrow.", []string{"cluster", "group", "topic", "partition", "owner", "clientid"}, nil)
	kafkaConsumerPartitionMaxOffsetDesc     = prometheus.NewDesc("kafka_burrow_partition_max_offset", "The log end offset on a partition as reported by burrow.", []string{"cluster", "group", "topic", "partition", "owner", "clientid"}, nil)
...
func (c *Collector) processGroup(cluster, group string) (metrics []prometheus.Metric) {
	...
	for _, partition := range resp.Status.Partitions {
		...
		labels := append(commonLabels, partition.Topic, strconv.Itoa(int(partition.Partition)), partition.Owner, partition.ClientID)
